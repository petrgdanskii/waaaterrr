...
def plot(u,v,h):
    ...
    b = np.zeros(129)
    h = np.insert(h, 0, values=b, axis=0)
    #xi = np.linspace(X.min(), X.max(), (len(h) / 3))
    #yi = np.linspace(Y.min(), Y.max(), (len(h) / 3))
    #zi = int.griddata((X, Y), h, (xi[None,:], yi[:,None]), method='cubic')


    ax = fig.add_subplot(111, projection='3d')
    ax.view_init(60, 50)
    ax.plot_surface(X, Y, h, cmap='inferno')
